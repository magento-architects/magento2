...
class Config implements \Magento\Framework\Interception\ConfigInterface
{
    ...
    public function initialize($classDefinitions = [])
    {
        ...
        foreach ($this->_scopeList->getAllScopes() as $scope) {
            list($scopeConfig, $scopeFiles) = $this->_reader->read($scope);
            $config = array_replace_recursive($config, $scopeConfig);
            $files = array_merge($files, $scopeFiles);
        }
        ...
        return [$this->_intercepted, $files];
    }
